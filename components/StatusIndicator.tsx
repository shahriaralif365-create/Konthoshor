...
export const StatusIndicator = memo(function StatusIndicator({ status, language = 'bengali' }: StatusIndicatorProps) {
  const translationKey = (language in translations) ? (language as Language) : 'english';
  const t = translations[translationKey].status;
  ...
  const getLabel = () => {
    if (status === 'listening' || status === 'restarting') {
      ...
      const currentLangName = langNames[language] || (language.charAt(0).toUpperCase() + language.slice(1));
      const baseLabel = status === 'restarting' ? 'Reconnecting' : t.listening;
      return `${baseLabel} (${currentLangName})`;
    }
